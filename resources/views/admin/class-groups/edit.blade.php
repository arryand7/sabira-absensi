                ...
                <form action="{{ route('admin.class-groups.update', $classGroup->id) }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="jenis_kelas" id="jenis_kelas"
                            ...
                            <option value="formal" {{ old('jenis_kelas', $classGroup->jenis_kelas) == 'formal' ? 'selected' : '' }}>Reguler</option>
                            <option value="muadalah" {{ old('jenis_kelas', $classGroup->jenis_kelas) == 'muadalah' ? 'selected' : '' }}>Non Reguler</option>
                    ...
                    <div>
                        ...
                        <select name="academic_year_id" id="academic_year_id"
                            ...
                            @foreach($academicYears as $year)
                                <option value="{{ $year->id }}" {{ old('academic_year_id', $classGroup->academic_year_id) == $year->id ? 'selected' : '' }}>
                                    {{ $year->name}}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex gap-4 mt-6">
                        <button type="submit"
                            class="bg-[var(--sabira-primary)] text-white px-4 py-2 rounded-md text-xs hover:bg-[var(--sabira-primary-active)] shadow inline-flex items-center gap-1">
