<x-app-shell>
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8 bg-[var(--sabira-surface-soft)]">
        <div class="flex items-center justify-between mb-2">
            <h1 class="text-2xl font-bold text-[var(--sabira-ink)]">
                <i class="bi bi-clipboard-check mr-2 text-[var(--sabira-muted)]"></i>
        ...
        <div class="mb-6 bg-[var(--sabira-surface-soft)] p-6 rounded-xl border border-[var(--sabira-border)] shadow-sm">
            ...
            <input type="text" id="search" placeholder="Ketik NIS siswa..."
            ...
            <div id="absensiList" class="space-y-3 max-h-96 overflow-y-auto pr-1">
                @foreach(\App\Models\Student::orderBy('nama_lengkap')->get() as $siswa)
                    <div class="flex items-center justify-between border border-[var(--sabira-border)] p-3 rounded-lg bg-white">
                        ...
                        <div
                            class="text-sm font-semibold cursor-pointer {{ ($absensiHariIni[$siswa->id]->status ?? 'alpa') === 'hadir' ? 'text-green-600' : 'text-red-600' }}"
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const searchInput = document.getElementById('search'); //pencarian nis dan nama
        const resultsDiv = document.getElementById('searchResults'); //hasil dari pencarian
        const absensiList = document.getElementById('absensiList'); //daftar nama yang dicari
        let currentFocus = -1; //nama yang dipilih di pencarian (highlight)
        let localStatusMap = {}; //simpan status dari sisi klien

        //yang bawah ini Mengirim data perubahan status via AJAX ke server Laravel. atau serverside --> revisi pak pur
        function toggleStatus(studentId) {
            ...
            fetch('{{ route('asrama.kegiatan.absen.update', ['id' => $kegiatan->id]) }}', {
                method: 'POST',
                headers: {
        ...
        // yang ini Memanggil toggleStatus() untuk mengubah status absensi siswa yang diklik.
        ...
        //yang bawah Jika input pencarian memiliki minimal 3 karakter: Melakukan request ke route Laravel (asrama.kegiatan.search) untuk mencari siswa. Hasil pencarian akan ditampilkan sebagai daftar pilihan.
        ...
        // ini buat Memungkinkan navigasi di daftar hasil pencarian menggunakan tombol up and down
        ...
        //bisa klik langsung status di daftar siswa untuk mengganti status tanpa harus cari siswa di kolom pencarian.
